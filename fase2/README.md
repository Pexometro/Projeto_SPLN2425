...
- `my_sentence_model_*/`: diretorias que contêm modelos treinados e métricas de validação (e.g.,

 `my_sentence_model_thr0.0_k5_e3_bs16/`).
...
## Configuração
1. Criar diretorias necessários:
   ```bash
...
- Treino com `CosineSimilarityLoss` por 3 a 10 epochs, com `batch_size` de 16 a 32.
- Hiperparâmetros testados: `threshold` (0.0, 0.05, 0.1), `top_k` (5, 10, 20), `epochs` (3, 5, 10), `batch_size` (16, 32).
- Modelos guardados em `my_sentence_model_thrX.X_kX_eX_bsX/`.
...
Foram treinados cinco modelos com diferentes hiperparâmetros e testados em cinco consultas. Em baixo apresentamos uma visão geral dos melhores resultados por consulta:
...
- **Cobertura do Dataset**: A consulta "inteligência artificial explicável" teve desempenho inferior devido à ausência de documentos relevantes no `ColDoc.json`.
